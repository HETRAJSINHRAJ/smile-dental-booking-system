...
import { Loader2, ArrowLeft, Calendar, Clock, User, FileText, CreditCard, Info } from 'lucide-react';
import { toast } from 'sonner';
import { useCurrency } from '@/lib/localization/useCurrency';
import { PaymentComponent } from '@/components/payment/PaymentComponent';
import { PaymentReceipt } from '@/components/payment/PaymentReceipt';
import { getPaymentConfig, formatPaymentBreakdown, getPaymentDescription } from '@/lib/payment/paymentConfig';
...
function ConfirmBookingContent() {
  ...
  const paymentBreakdown = service ? formatPaymentBreakdown(service.price, paymentConfig) : null;
  ...
  const handlePaymentSuccess = async (response: any) => {
    ...
    try {
      ...
      const transactionId = response.razorpay_payment_id || response.transaction_id || response.paymentIntent?.id || 'N/A';
      ...
      await paymentAuditService.logPaymentEvent({
        appointmentId,
        patientId: user.uid,
        patientName: user.displayName || 'Not provided',
      ...
      // Show receipt instead of redirecting immediately
      setShowReceipt(true);
    } catch (error) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mx-auto text-center mb-12">
          ...
          <div className="inline-flex items-center justify-center w-16 h-16 bg-muted rounded-full mb-4">
            {showPayment ? <CreditCard className="w-8 h-8 text-primary" /> : <FileText className="w-8 h-8 text-primary" />}
          </div>
          <h1 className="text-4xl font-bold mb-4">
            {showPayment ? 'Complete Payment' : 'Confirm Your Appointment'}
          </h1>
          <p className="text-xl text-muted-foreground mb-8">
            {showPayment ? 'Step 4 of 4: Secure payment processing' : 'Step 4 of 4: Review and confirm your booking'}
          </p>
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="mb-6">
              ...
              {paymentResponse && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-green-800">Payment successful! Processing your appointment...</p>
                </div>
              )}
            </div>
          )}
          ...
          {!showPayment && paymentBreakdown && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium">{formatCurrency(paymentBreakdown.appointmentReservationFee)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-700">GST (18%):</span>
                  <span className="font-medium">{formatCurrency(paymentBreakdown.appointmentTax)}</span>
                </div>
                <div className="border-t border-blue-200 pt-2 flex justify-between items-center font-semibold">
                  ...
                  <span className="text-blue-600">{formatCurrency(paymentBreakdown.appointmentTotal)}</span>
                </div>
                
                {!paymentConfig.enableServicePaymentOnline && (
                  <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      <strong>Service Payment:</strong> The service fee of {formatCurrency(paymentBreakdown.serviceTotal)} 
            ...
            {/* Payment Receipt */}
            {showReceipt && paymentResponse && paymentBreakdown && service && provider && user && (
              <div className="space-y-6">
                <PaymentReceipt
                  appointmentData={{
                    id: paymentResponse.appointmentId || 'temp',
                    serviceName: service.name,
                    providerName: provider.name,
                    date: date!,
                    time: time!,
                  ...
                  paymentData={{
                    transactionId: paymentResponse.razorpay_payment_id || paymentResponse.transaction_id || 'N/A',
                    amount: paymentBreakdown.appointmentReservationFee,
                    taxAmount: paymentBreakdown.appointmentTax,
                    totalAmount: paymentBreakdown.appointmentTotal,
                    paymentMethod: paymentResponse.payment_method || 'online',
                    paymentDate: new Date().toISOString(),
                    paymentDescription: 'Appointment Reservation Fee'
                  }}
                  servicePaymentInfo={!paymentConfig.enableServicePaymentOnline ? {
                    serviceAmount: paymentBreakdown.servicePrice,
                    serviceTax: paymentBreakdown.serviceTax,
                    serviceTotal: paymentBreakdown.serviceTotal,
                    paymentDue: 'At clinic during visit'
                  } : undefined}
                />
